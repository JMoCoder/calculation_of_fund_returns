...
## [3.3.0] - 2024-12-22 - 🛠️ 项目结构与开发工具优化版
...
#### 📁 项目结构标准化
- **🔄 测试系统重构**:
  - 将 `run_tests.py` 移动到 `tests/` 目录下
  - 统一测试文件管理和组织结构
  - 修复所有import路径引用问题
  - 更新项目结构文档和使用说明

- **📚 文档体系完善**:
  - 新增 `document/DEPENDENCIES.md` 依赖管理说明
  - 将 `TOOLS_README.md` 移动到 `document/` 目录下统一管理
  - 保留 `chart_copy_diagnostic.html` 图表复制诊断工具在根目录
  - 建立标准化的文档目录结构
  - 更新README中的工具使用章节

#### 🗂️ 文档组织优化
- **📋 document/ 目录统一管理**:
  - `document/TOOLS_README.md` - 开发工具使用指南 (已移动)
  - `document/DEPENDENCIES.md` - 依赖管理说明
  - `document/PROJECT_STRUCTURE.md` - 项目结构文档
  - 实现文档的集中化管理和维护
...
- **🎨 用户体验改进**:
  - 移除emoji字符解决Windows编码问题
  - 使用ASCII字符替代表情符号
  - 改进日志输出格式和状态提示
  - 保持功能完整性和可读性

#### 📦 依赖管理标准化
- **📋 requirements.txt位置保持**:
  - 遵循Python生态标准约定
  - 保持在项目根目录不移动
  - 维护所有引用脚本的兼容性
  - 支持pip、CI/CD和IDE自动识别
...
- **🛠️ 工具使用指南**:
  - 将TOOLS_README.md移动到document目录统一管理
  - 详细的使用方法和适用场景
  - 维护建议和最佳实践
  - 问题排查和技术支持指南
...
├── 📄 app.py                          # 主应用程序
├── 📋 requirements.txt                # Python依赖定义 (保持根目录)
├── 📊 chart_copy_diagnostic.html      # 图表复制诊断工具
├── 📁 tests/                         # 测试目录
│   ├── 🚀 run_tests.py               # 统一测试运行器 (已移动)
│   ├── test_api.py                   # API接口测试
│   ├── test_calculations.py          # 计算逻辑测试
│   ├── test_charts.py                # 图表功能测试
│   └── test_export.py                # 导出功能测试
├── 📁 document/                      # 📚 项目文档目录 (统一管理)
│   ├── TOOLS_README.md               # 🛠️ 开发工具使用指南 (已移动)
│   ├── DEPENDENCIES.md               # 📦 依赖管理说明
│   └── PROJECT_STRUCTURE.md          # 🏗️ 项目结构文档
├── 📁 templates/                     # HTML模板
├── 📁 static/                        # 静态资源
└── 📁 .DOC/                          # 开发文档
```
...
#### 📋 文件移动记录
- ✅ `run_tests.py`: 根目录 → `tests/run_tests.py`
- ✅ `TOOLS_README.md`: 根目录 → `document/TOOLS_README.md` (新增移动)
- ✅ `DEPENDENCIES.md`: 新建于 `document/DEPENDENCIES.md`
- ✅ `requirements.txt`: 保持在根目录 (标准约定)
- ✅ `__pycache__/`: 已清理删除
...
python tests/run_tests.py --no-server-check  # 跳过服务器检查
```

#### 📚 文档引用更新
- 更新README.md中所有测试运行器引用
- 修正项目结构图和使用说明
- 添加开发工具章节和诊断工具说明
- 完善技术文档和维护指南
...
#### 🚫 零破坏性变更
- **不影响**: 所有现有功能正常工作
- **兼容**: deploy.bat和CI/CD脚本无需修改
- **保持**: requirements.txt位置和引用
- **维护**: 向后兼容性和用户习惯
